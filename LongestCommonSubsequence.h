...
public:
    ...
    // Time: could be exponential, Space: O(|x|*|y|)
    vector<string> backtraceAll(const string &x, const string &y, const vector<vector<int> > &DP, int i, int j) {
        if (i == 0 || j == 0)
            return {""};
        if (x[i-1] == y[j-1]) {
            vector<string> svec = backtraceAll(x, y, DP, i-1, j-1);
            for (auto &s : svec)
                s.push_back(x[i-1]);
            return svec;
        }
        vector<string> svec;
        if (DP[i][j] == DP[i][j-1])
            svec = backtraceAll(x, y, DP, i, j-1);
        if (DP[i][j] == DP[i-1][j]) {
            vector<string> svec2 = backtraceAll(x, y, DP, i-1, j);
